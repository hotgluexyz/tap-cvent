# AGENTS.md - AI Agent Development Guide for tap-cvent
...
This tap follows the Singer specification and uses the Hotglue Singer SDK (hotglue_singer_sdk) to extract data from Cvent.

### Key Components

1. **Tap Class** (`tap_cvent/tap.py`): Main entry point, defines streams and configuration
2. **Client** (`tap_cvent/client.py`): HTTP client, stream base class, and (where applicable) how auth is attached to requests
3. **Streams** (`tap_cvent/streams.py`): Stream classes, schemas (`th.PropertiesList`), and—depending on stream type—`path`, `query`, or custom extraction hooks
4. **Authentication** (`tap_cvent/auth.py`): `CventAuthenticator` subclasses the SDK’s `OAuthAuthenticator` (e.g. `oauth_request_body` for the token endpoint). `tap.py` exposes **`access_token_support()`** so Hotglue can coordinate tokens; `client.py` builds the authenticator via that hook.
...
1. Add a stream class in `tap_cvent/streams.py` (follow naming: PascalCase + `Stream`, Singer **`name`** in snake_case—consistent with existing streams).
...
5. Add the class to **`STREAM_TYPES`** in `tap.py` and to the **`from tap_cvent.streams import (...)`** block (Cookiecutter pre-fills both from **`stream_names`**).
...
The agent should align **`name`**, **`path`** (or **`query`** for GraphQL), **`schema`**, **`records_jsonpath`** / **`get_next_page_token()`** in **`tap_cvent/client.py`** when needed, and **`STREAM_TYPES`** + imports in **`tap.py`** for new streams. See **Handling Pagination** and **Adding a New Stream** above.
...
- **`auth.py`**: `CventAuthenticator` extends `OAuthAuthenticator`; implement **`oauth_request_body`** for the vendor’s token request (grant type, scope, etc.).
...
```bash
tap-cvent --config config.json --discover
tap-cvent --config config.json --catalog catalog.json
```

### Keeping tap config, docs, and env in sync

Authoritative settings live in **`config_jsonschema`** on the tap class in `tap_cvent/tap.py` (Hotglue Singer SDK). Everything operators and Hotglue jobs read should match that schema.
...
1. Update `config_jsonschema` in `tap_cvent/tap.py`.
2. Update **`README.md`**: the configuration table, the example `config.json`, and any prose that names settings.
3. Update **`.env.example`** so variable names stay aligned with `tap-cvent --about` / `--config=ENV` (keys are derived from config property names).
...
TAP_CVENT_API_URL=https://api.example.com
TAP_CVENT_ACCESS_KEY=your_access_key_here
TAP_CVENT_BATCH_SIZE=100
```
...
- Prefer `tap-cvent --about` (or `--format=markdown`) as the generated reference for your tree.
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`tap-cvent --version` with your venv activated), Python version, and a minimal reproduction case when reporting bugs.
